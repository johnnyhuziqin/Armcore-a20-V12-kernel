...
static int parser_atoi(const char *p)
{
       int i = 0;
...
script_get_item(char *main_key, char *sub_key, script_item_u *item)
{
    ...
    while (mainkey) {
        ...
        if ((mainkey->hash == main_hash) && !strcmp(mainkey->name, main_key)) {
            /* find the main key */
            if (strcmp(main_key, "disp_init") == 0 || strcmp(main_key, "lcd0_para") == 0){
                smdt_param = 0;
	    }
