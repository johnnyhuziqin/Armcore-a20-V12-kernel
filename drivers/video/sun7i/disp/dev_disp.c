...
static ssize_t show_output_x(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", BSP_disp_get_screen_width(0));
}

static ssize_t show_output_type(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n",g_fbi.disp_init.output_type[0]);
}

static ssize_t show_output_mode(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = -1;
	if (g_fbi.disp_init.output_type[0] == DISP_OUTPUT_TYPE_HDMI)
		ret = g_fbi.disp_init.tv_mode[0];
	else if (g_fbi.disp_init.output_type[0] == DISP_OUTPUT_TYPE_TV)
		ret = g_fbi.disp_init.tv_mode[0];
	else if (g_fbi.disp_init.output_type[0] == DISP_OUTPUT_TYPE_VGA)
		ret = g_fbi.disp_init.vga_mode[0];
	else if (g_fbi.disp_init.output_type[0] == DISP_OUTPUT_TYPE_LCD)
		ret = -1;
	else
		ret = -1;
...
static DEVICE_ATTR(output_x, S_IRUGO | S_IWUSR, show_output_x, NULL);
static DEVICE_ATTR(output_type, S_IRUGO | S_IWUSR, show_output_type, NULL);
static DEVICE_ATTR(output_mode, S_IRUGO | S_IWUSR, show_output_mode, NULL);
static struct attribute *dev_attrs[] = {
            &dev_attr_output_x.attr,
