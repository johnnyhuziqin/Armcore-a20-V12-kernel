...
#ifdef CONFIG_TOUCHSCREEN_USBIRTOUCH_TUOJIUZHE
#define LCD_X_SIZE     4096
#define LCD_Y_SIZE     4096
#define TP_H           2409
#define TP_W           4309
#define TOUCH_NAME    "usbtouchscreen_tuojiuzhe"
...
static const struct usb_device_id usbtouch_devices[] = {
	...
	{USB_DEVICE(0x1130, 0x1650), .driver_info = DEVTYPE_IRTOUCH},
#endif
#ifdef CONFIG_TOUCHSCREEN_USBIRTOUCH_TUOJIUZHE
	...
	{USB_DEVICE(0x1aad, 0x0004), .driver_info = DEVTYPE_GENERAL_TOUCH},
	{USB_DEVICE(0x1aad, 0x0001), .driver_info = DEVTYPE_GENERAL_TOUCH},
#endif
...
#endif
#ifdef CONFIG_TOUCHSCREEN_USBIRTOUCH_TUOJIUZHE
static int irtouch_tuojiuzhe_read_data(struct usbtouch_usb *dev, unsigned char *pkt)
{
	#if 0
//	dev->x = (pkt[3] << 8) | pkt[2];
	dev->x = (pkt[4] << 8) | pkt[3];
...
//	dev->y = (pkt[5] << 8) | pkt[4];
	dev->y = (pkt[6] << 8) | pkt[5];
	p2_y = (pkt[12] << 8) | pkt[11];
	#endif
	dev->x=LCD_X_SIZE * ((pkt[4] << 8) | pkt[3]) / TP_W;
	dev->y= LCD_Y_SIZE * ((pkt[6] << 8) | pkt[5]) / TP_H;
	p2_x=LCD_X_SIZE * ((pkt[10] << 8) | pkt[9]) / TP_W ;
	p2_y=LCD_Y_SIZE * ((pkt[12] << 8) | pkt[11]) / TP_H ;
	
	dev->touch = ((pkt[1] & 0x03) ||(pkt[7] & 0x03))? 1 : 0;
	//printk("pkt[3]=%d pkt[4]=%d pkt[5]=%d pkt[6]=%d pkt[10]=%d pkt[11]=%d pkt[12]=%d pkt[13]=%d pkt[1]=%d pkt[7]=%d\n",pkt[3],pkt[4],pkt[5],pkt[6],pkt[10],pkt[11],pkt[12],pkt[13],pkt[1],pkt[7]);
	return 1;
}
#endif
/*****************************************************************************
...
static struct usbtouch_device_info usbtouch_dev_info[] = {
	...
	[DEVTYPE_IRTOUCH] = {
		...
		.max_yc		= 0x0fff,
		.rept_size	= 32,
		.read_data	= irtouch_tuojiuzhe_read_data,
	},
#endif
#ifdef CONFIG_TOUCHSCREEN_USB_IDEALTEK
	[DEVTYPE_IDEALTEK] = {
...
//	if (swap_xy) {
	...
	//printk("pkt[13]=%d usbtouch->x=%d usbtouch->y= %d p2_x=%d p2_y=%d\n",pkt[13],usbtouch->x,usbtouch->y,p2_x,p2_y);
		if(pkt[13]==2)
		{
			...
			input_mt_sync(usbtouch->input);
			input_report_abs(usbtouch->input, ABS_MT_POSITION_Y, p2_y);
			input_report_abs(usbtouch->input, ABS_MT_POSITION_X, p2_x);
			input_report_abs(usbtouch->input, ABS_MT_TOUCH_MAJOR, 1);
			input_report_abs(usbtouch->input, ABS_MT_WIDTH_MAJOR, 1);
			input_mt_sync(usbtouch->input);
		}
