...
/* support mouse advance 2013-04-25 */
...
/* flicker led when receive ir signal 2013-04-24 */
	...
	ir_led = val.gpio.gpio;
	if(0 != gpio_request(val.gpio.gpio, NULL)) {
...
/* add by cjcheng end   -----------------------------------} */
			...
			if((code==IR_REPEAT_CODE)||(code_valid)) {  //Error, may interfere from other source
                ...
                mod_timer(s_timer, jiffies + (HZ/8));
			...
			if(code_valid) {
                ...
                s_timer->expires = jiffies + (HZ/8);  //250ms timeout
                /* modify by cjcheng end   -----------------------------------} */
				add_timer(s_timer);
				timer_used = 1;
			}
